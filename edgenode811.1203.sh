...
CLIENT=cvs
CONTAINER=https://dmcontainerresources.blob.core.windows.net/$CLIENT
RPDM_VERSION=8.1.1.1203
...
JAVA_PLUGINS=javaplugins.tar.gz
SFTPCERT=redpointopscert.tar.gz

CASSINSTALLER=RedPointDM-CASS-1_8_1-2018-11-01-r20181109000956-for-Linux.tgz 
GEOCODERINSTALLER=RedPointDM-Geocoder-2017-09-r20180323062640-for-Linux.tgz

GeocoderInstaller_Dir=DMGeocoder_2017-12
CassInstaller_Dir=DMCass_2018-11
...
if [ ! -e "$GEOCODERINSTALLER" ]; then
	echo -n "Downloading $GEOCODERINSTALLER from $CONTAINER..."
	sudo wget -o wget-geo.log $CONTAINER/$GEOCODERINSTALLER
	echo " done."
fi
if [ ! -e "$SFTPCERT" ]; then
	echo -n "Downloading $SFTPCERT from $CONTAINER..."
	sudo wget -o wget-sftp.log $CONTAINER/$SFTPCERT
	echo " done."
fi
if [ ! -e "$CASSINSTALLER" ]; then
	echo -n "Downloading $CASSINSTALLER from $CONTAINER..."
	sudo wget -o wget-cass.log $CONTAINER/$CASSINSTALLER
	echo " done."
fi

if [ ! -e "$JAVA_PLUGINS" ]; then
	echo -n "Downloading $JAVA_PLUGINS from $CONTAINER..."
	sudo wget -o wget-java.log $CONTAINER/$JAVA_PLUGINS
	echo " done."
fi
...
if [ ! -d "$RPDM_INSTALLER_DIR" ]; then
	...
	echo -n "Unpacking $CassInstaller..."
    cd $RPDM_INSTALLER_DIR
	tar xf $CassInstaller
	echo " done."
	echo -n "Installing RPDM $CassInstaller..."
	cd $CassInstaller_Dir \
		&& ./install.pl -y
	echo " done."

    cd $RPDM_INSTALLER_DIR
	echo -n "Unpacking $GeocoderInstaller..."
	tar xf $GeocoderInstaller
	echo " done."
	echo -n "Installing RPDM $GeocoderInstaller..."
	cd $GeocoderInstaller_Dir \
		&& ./install.pl -y
	echo " done."
